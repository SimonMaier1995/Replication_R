...
# 25 11 7
# 29% 12% 8%
# noise threshold from control 0.057232

# Calculate avg. lambda ----

## stat2701
# number of participants showing no signs of debt aversion
# the first argument of aux is the number of participants with lambda=0
aux[1]/sum(aux)
#0.5116279

# the second argument of aux is the number of participants with low lambda
aux[2]/sum(aux)
#0.255814

# the third argument of aux is the number of participants with high lambda
aux[3]/sum(aux)
#0.2325581

# avg. lambda no thresold
meanLambda <- (aux[1]*1+aux[2]*1.04+aux[3]*1.14)/nrow(MSE_lowDebt)
# 1.042791
...
# stat 2801,2802,2803
## share of participants unclassified
# previous number of participants is 86
sum(aux)/86
# 0.4767442
...
# the second and third argument of aux  are participants who are debt averse
(aux[2]+aux[3])/sum(aux)
# 0.4146341


meanLambdaThreshold <- (aux[1]*1+aux[2]*1.04+aux[3]*1.14)/nrow(MSE_lowDebt[MSE_lowDebt$typeMSE<=0.054,])
# 1.033659
...
for (i in 1:length(thresholdValues)) {
  ...
  lambdaMatrix[i,1] <- (aux[1]*1+aux[2]*1.04+aux[3]*1.14)/nrow(MSE_lowDebt[MSE_lowDebt$typeMSE<=threshold,])
  lambdaMatrix[i,2] <- threshold
}

# avg. lambda with perfect fit, no type 3 so previous gives NA, but all MPSE>0 have a type 3
aux <- as.vector(table(MSE_lowDebt$type[MSE_lowDebt$typeMSE==0]))

meanLambdaThreshold <- (aux[1]*1+aux[2]*1.04)/nrow(MSE_lowDebt[MSE_lowDebt$typeMSE==0,])
# 1.006154
lambdaMatrix[1,1] <- (aux[1]*1+aux[2]*1.04)/nrow(MSE_lowDebt[MSE_lowDebt$typeMSE==0,])

lambdaMatrix <- as.data.frame(lambdaMatrix)

write.table(lambdaMatrix, "C:/Users/am04817/Dropbox/Behavioral debt experiment/NewDesign/StructuralEstimation/lambdaMatrix.txt",
            sep="\t", row.names = F)
