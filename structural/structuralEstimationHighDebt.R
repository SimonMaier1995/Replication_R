...
### Structural Estimation High Debt, Classify Subjects According to Types
...
data_set <- subset(mainData, subset = treatment %in% "High Debt")
rm(mainData)
...
aux$initialBalance3[aux$day=="Day 1"] <- -2900
aux$initialBalance4[aux$day=="Day 1"] <- -3500
...
MSE_lowDebt$treatment <- "High Debt"
...
# 65 18 3
# 0.7441860 0.1395349 0.1162791 
table(MSE_lowDebt$type[MSE_lowDebt$typeMSE<=0.057232])
...
# 29 5 2
# 0.33720930 0.05813953 0.02325581 
...
meanLambdaThreshold <- (aux[1]*1+aux[2]*lowLambda+aux[3]*highLambda)/nrow(MSE_lowDebt[MSE_lowDebt$typeMSE==0,])
# 1.0128
lambdaMatrix[1,1] <- (aux[1]*1+aux[2]*lowLambda+aux[3]*highLambda)/nrow(MSE_lowDebt[MSE_lowDebt$typeMSE==0,])

lambdaMatrix <- as.data.frame(lambdaMatrix)

write.table(lambdaMatrix, "C:/Users/am04817/Dropbox/Behavioral debt experiment/NewDesign/StructuralEstimation/lambdaMatrixHighDebt.txt",
            sep="\t", row.names = F)
